...
public:
    ...
    {   // vy increases linearly with time with an acceleration of acc
        int acc=35;
        vy=vy+acc*t;
        // move the bubble
        ...
        if((vy >0 && new_y > (0+WINDOW_Y-50-circle.getRadius()) // bounce along y direction upon encountering the floor
         || (vy <0 && new_y< (0+circle.getRadius())))) // bounce along  y direction upon encountering the ceiling
        {
            vy=-vy;
